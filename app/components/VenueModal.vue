<template>
  <div v-if="ui.showModal" class="venue-modal-backdrop" @click.self="ui.hideVenueModal()" @keydown.esc="ui.hideVenueModal()">
    <div class="venue-modal z-5" role="dialog" aria-modal="true">
      <button type="button" class="venue-modal-close" aria-label="Close" @click="ui.hideVenueModal()">
        &times;
      </button>
      <section v-if="venue" class="venue">
        <div class="venue-logo-title">
          <img :src="venue.logo_url" class="venue-logo" alt="">
          <h1 class="venue-title">
            {{ venue.name }}
          </h1>
        </div>
        <div class="venue-info">
          <a :href="mapsUrl" class="venue-info__link venue-info__link--address" target="_blank">
          ...
          <a v-if="venue.website" :href="venue.website" class="venue-info__link venue-info__link--website" target="_blank" rel="noreferrer">
            <span class="icon-prefix">
          ...
          <a v-if="venue.email" :href="`mailto:${venue.email}`" class="venue-info__link venue-info__link--email">
            <span class="icon-prefix">
              <svg style="width:24px;height:24px" viewBox="0 0 24 24">
          ...
          <a v-if="venue.phone_number" :href="`tel:${venue.phone_number}`" class="venue-info__link venue-info__link--phone">
            <span class="icon-prefix">
              <svg style="width:24px;height:24px" viewBox="0 0 24 24">
          ...
          <div class="venue-social">
            <a v-if="untappd" :href="untappd" class="btn venue-untappd" target="_blank" rel="noreferrer">
            ...
            <a v-if="facebook" :href="facebook" class="btn venue-facebook" target="_blank" rel="noreferrer">
              <svg class="venue-social__icon" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <a v-if="instagram" :href="instagram" class="btn venue-instagram" target="_blank" rel="noreferrer">
              <svg class="venue-social__icon" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <a v-if="twitter" :href="twitter" class="btn venue-twitter" target="_blank" rel="noreferrer">
              <svg class="venue-social__icon" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
...
<script setup>
const ui = useUiStore()
const venue = computed(() => ui.modalVenue)

const mapsUrl = computed(() => `https://maps.google.com/?q=${venue.value.address}, ${venue.value.city}, ${venue.value.state}, ${venue.value.postal_code}`)
const facebook = computed(() => venue.value.facebook_page)
const instagram = computed(() => venue.value.instagram_handle ? `https://instagram.com/${venue.value.instagram_handle}` : null)
const twitter = computed(() => venue.value.twitter_handle ? `https://twitter.com/${venue.value.twitter_handle}` : null)
const untappd = computed(() => venue.value.untappd_url)

const formattedPhone = computed(() => {
  const regex = /(\d\d\d)(\d\d\d)(\d\d\d\d)/
  if (venue.value.phone_number && venue.value.phone_number.match(regex)) {
    return venue.value.phone_number.replace(regex, '($1) $2-$3')
  }
  return venue.value.phone_number
})

const formattedWebsite = computed(() => {
  const regex = /http(s)?:\/\/(www.)?/
  if (venue.value.website && venue.value.website.match(regex)) {
    const formatted = venue.value.website.replace(regex, '')
    if (formatted.endsWith('/')) {
      return formatted.substr(0, formatted.length - 1)
    }
    return formatted
  }
  return venue.value.website
})
</script>
